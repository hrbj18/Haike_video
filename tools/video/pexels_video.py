...
class PexelsVideo(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "exclude_video_ids": {
                ...
                "description": "Pexels video IDs that must not be selected",
            },
            "preferred_quality": {
                "type": "string",
                "enum": ["hd", "sd"],
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            excluded_ids = {str(value) for value in inputs.get("exclude_video_ids", []) if value is not None}
            video = next((item for item in videos if str(item.get("id")) not in excluded_ids), None)
            if video is None:
                return ToolResult(
                    success=False,
                    error=f"All returned videos were already used for query: {query}",
                    data={
