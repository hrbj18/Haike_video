...
def _ffmpeg_pair_in(directory: Path) -> tuple[str, str] | None:
    """Return a complete local FFmpeg/ffprobe pair from one directory."""
...
def _discover_ffmpeg_pair() -> tuple[str, str] | None:
    """Find FFmpeg and ffprobe without importing or downloading a runtime."""

    path_ffmpeg = shutil.which("ffmpeg")
    path_ffprobe = shutil.which("ffprobe")
    if path_ffmpeg and path_ffprobe:
        return str(Path(path_ffmpeg).resolve()), str(Path(path_ffprobe).resolve())

    # ``static_ffmpeg.add_paths()`` may download binaries.  Preflight must stay
    # read-only, so inspect the installed package layout through find_spec only.
    try:
    ...
    if spec is not None:
        for location in spec.submodule_search_locations or ():
            ...
            for candidate in (package_root / "bin" / platform_dir, package_root / "bin"):
                pair = _ffmpeg_pair_in(candidate)
                if pair:
                    return pair
    ...
    if local_app_data:
        ...
        try:
            matches = sorted(package_root.glob("Gyan.FFmpeg.Essentials_*/*/bin"))
        except OSError:
            matches = []
...
def _ensure_ffmpeg_on_path() -> str | None:
    """Resolve a usable FFmpeg/ffprobe pair and make it discoverable.

    Winget installs the Gyan FFmpeg package into a user-scoped directory on
    Windows, but deliberately does not always add that directory to PATH.  A
    composition should not be reported as unavailable merely because the
    desktop host has not restarted after that install.  Once found, prepend
    the binary directory so ``ffmpeg`` and ``ffprobe`` calls made by this
    module (and its subprocesses) share one deterministic runtime.
    """
    ...
    known = {part.lower() for part in current_path.split(os.pathsep) if part}
    prepend: list[str] = []
    for binary in pair:
        binary_dir = str(Path(binary).parent)
        if binary_dir.lower() not in known:
            prepend.append(binary_dir)
            known.add(binary_dir.lower())
    if prepend:
        os.environ["PATH"] = os.pathsep.join([*prepend, current_path])
    return pair[0]


def _ensure_npx_on_path() -> str | None:
    """Resolve the repository's bundled Node launcher when PATH is minimal.

    Desktop-launched Python processes do not necessarily inherit the shell
    PATH that contains Node.  The project ships a tiny ``.local-bin/npx.cmd``
    wrapper for exactly that case; prepend only this explicit, repository-local
    launcher rather than guessing a global Node installation.
    """
    bundled_node = Path.home() / ".cache" / "codex-runtimes" / "codex-primary-runtime" / "dependencies" / "node" / "bin"
    if bundled_node.is_dir() and not shutil.which("node"):
        current_path = os.environ.get("PATH", "")
        os.environ["PATH"] = str(bundled_node) + os.pathsep + current_path
    found = shutil.which("npx")
    if found:
        return found
    local_bin = Path(__file__).resolve().parent.parent.parent / ".local-bin"
    launcher = local_bin / ("npx.cmd" if os.name == "nt" else "npx")
    if not launcher.is_file():
        return None
    current_path = os.environ.get("PATH", "")
    if str(local_bin).lower() not in {part.lower() for part in current_path.split(os.pathsep) if part}:
...
class VideoCompose(BaseTool):
    ...
    def _ffmpeg_available(self) -> bool:
        """Report availability only when a complete local tool pair exists."""
        return _discover_ffmpeg_pair() is not None
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        # Resolve the local runtime before any operation uses ``ffmpeg`` or
        # ``ffprobe`` by name.  This is intentionally harmless for operations
        # that later return a structured availability error.
    ...
    def _has_audio_stream(path: Path) -> bool:
        ...
        pair = _discover_ffmpeg_pair()
        if pair is None:
            raise RuntimeError("ffprobe unavailable: a complete FFmpeg/ffprobe pair is required")
        try:
            proc = subprocess.run(
                [
                    pair[1], "-v", "error",
        ...
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or "unknown ffprobe failure").strip()
            raise RuntimeError(f"ffprobe failed while inspecting audio streams: {detail[:800]}")
        return any(line.strip() == "audio" for line in (proc.stdout or "").splitlines())
            ...
            if subtitle_path and Path(subtitle_path).exists():
                ...
                vfilters.append(
                    self._build_subtitles_filter(
                        subtitle_path,
                        style,
                        original_size=f"{target_w}x{target_h}",
                    )
                )
    ...
    def _burn_subtitles(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        style = inputs.get("subtitle_style", {})
        original_size = str(inputs.get("original_size") or "1920x1080").strip()
        codec = inputs.get("codec", "libx264")
        crf = inputs.get("crf", 23)
        ...
        cmd = [
            "ffmpeg", "-y",
            "-i", str(input_path),
            "-vf", self._build_subtitles_filter(subtitle_path, style, original_size),
    ...
    def _overlay(self, inputs: dict[str, Any]) -> ToolResult:
        """Composite globally timed overlay images/videos on a base video.

        Every overlay input owns a zero-based local timeline.  Its PTS must be
        shifted to ``start_seconds`` before framesync; an ``enable`` expression
        alone only hides frames and does not delay playback.  Without this
        shift a later presenter clip reaches EOF before its scene begins and
        FFmpeg repeats a frozen terminal frame.
        """
        input_path = Path(inputs["input_path"])
        ...
        for i, ov in enumerate(overlays):
            ...
            x = int(ov.get("x", 0))
            y = int(ov.get("y", 0))
            start = max(0.0, float(ov.get("start_seconds", 0) or 0))
            raw_end = ov.get("end_seconds")
            ...
            if is_image:
                input_args.extend(["-loop", "1", "-i", str(asset_path)])
            else:
            ...
            if duration is not None:
                preparation.append(f"trim=duration={duration:.6f}")
            preparation.append(f"setpts=PTS-STARTPTS+{start:.6f}/TB")
            if "width" in ov and "height" in ov:
                w = int(ov["width"])
                h = int(ov["height"])
                preparation.append(f"scale={w}:{h}")
                shape = str(ov.get("shape") or "rectangle").strip().lower()
                if shape in {"circle", "rounded"}:
                    if shape == "circle":
                        alpha = "if(lte((X-W/2)*(X-W/2)+(Y-H/2)*(Y-H/2),(min(W,H)/2)*(min(W,H)/2)),255,0)"
                    else:
                        radius = max(4, int(min(w, h) * .08))
                        alpha = (
            ...
            filter_parts.append(f"[{overlay_input}]{','.join(preparation)}[{prepared_label}]")
    ...
    def _resolve_subtitle_style(
        ...
        if edit_decisions:
            ...
            # Older artifact schemas used ``style`` for the display-mode
            # string (sentence / karaoke).  Newer workbenches correctly use
            # it as an ASS style object.  Treat the legacy value as metadata
            # instead of crashing a render on ``.items()``.
    ...
    def _build_subtitles_filter(
        ...
        style: dict,
        original_size: str = "1920x1080",
    ) -> str:
        """Build an ASS filter with a native canvas for every aspect ratio.

        SRT has no canvas metadata. Passing it directly to libass makes the
        renderer invent a reference canvas, which can stretch or compress
        glyphs when a portrait output is requested. We convert the cues to a
        project-local ASS file with PlayResX/PlayResY matching the final
        output, and explicitly keep ScaleX/ScaleY at 100.
        """
        width, height = VideoCompose._parse_dimensions(original_size)
        ass_path = Path(subtitle_path).with_suffix(".ass")
        VideoCompose._write_ass_subtitles(Path(subtitle_path), ass_path, style, width, height)
    ...
    @staticmethod
    def _responsive_ass_style(style: dict, width: int, height: int) -> dict[str, Any]:
        """Calculate readable subtitle metrics in final-video pixels."""
        short_side = min(width, height)
        responsive = style.get("responsive", True) is not False
        if responsive:
            try:
                font_ratio = float(style.get("font_size_ratio", 0.045))
            except (TypeError, ValueError):
                font_ratio = 0.045
            font_ratio = max(0.025, min(0.085, font_ratio))
            font_size = max(24, min(96, round(short_side * font_ratio)))
            try:
                width_ratio = float(style.get("caption_width_ratio", 0.88))
            except (TypeError, ValueError):
                width_ratio = 0.88
            width_ratio = max(0.42, min(0.94, width_ratio))
            margin_v = max(24, round(height * (0.115 if height > width else 0.075)))
            margin_lr = max(0, round(width * (1 - width_ratio) / 2))
            outline_width = max(1, round(font_size * 0.06))
        else:
            font_size = int(style.get("font_size") or 28)
            margin_v = int(style.get("margin_v") or 40)
            margin_lr = int(style.get("margin_lr") or 40)
            outline_width = int(style.get("outline_width") or 2)
        return {
            "font": style.get("font") or "Microsoft YaHei",
            "font_size": font_size,
            "bold": bool(style.get("bold", True)),
            "primary_color": style.get("primary_color") or "&H00FFFFFF",
            "secondary_color": style.get("secondary_color") or "&H00FFFFFF",
            "outline_color": style.get("outline_color") or "&HAA000000",
            "back_color": style.get("back_color") or "&H80000000",
            ...
            "margin_lr": margin_lr,
            "margin_v": margin_v,
            "position_x_ratio": style.get("position_x_ratio"),
            "position_y_ratio": style.get("position_y_ratio"),
        }
    ...
    def _write_ass_subtitles(
        ...
        metrics = VideoCompose._responsive_ass_style(style, width, height)
        scene_styles: list[tuple[float, float, str, dict[str, Any]]] = []
        for index, raw in enumerate(style.get("scene_styles", []) if isinstance(style, dict) else []):
            if not isinstance(raw, dict) or not isinstance(raw.get("style"), dict):
                continue
            try:
            ...
            if end <= start:
                continue
            name = f"Scene{index + 1:03d}"
            scene_styles.append((start, end, name, VideoCompose._responsive_ass_style(raw["style"], width, height)))

        def style_line(name: str, item: dict[str, Any]) -> str:
            return (
                f"Style: {name},{item['font']},{item['font_size']},{item['primary_color']},"
                f"{item['secondary_color']},{item['outline_color']},{item['back_color']},"
                f"{1 if item['bold'] else 0},0,0,0,100,100,0,0,{item['border_style']},"
                f"{item['outline_width']},{item['shadow']},{item['alignment']},"
                f"{item['margin_lr']},{item['margin_lr']},{item['margin_v']},134"
            )

        def style_for_time(seconds: float) -> tuple[str, dict[str, Any]]:
            for start, end, name, item in scene_styles:
                if start - .001 <= seconds < end + .001:
                    return name, item
        ...
        for block in blocks:
            ...
            start_ms = ((values[0] * 3600 + values[1] * 60 + values[2]) * 1000) + values[3]
            end_ms = ((values[4] * 3600 + values[5] * 60 + values[6]) * 1000) + values[7]
            text = "\\N".join(lines[time_index + 1:]).strip()
            if not text:
                continue
            text = text.replace("{", "\\{").replace("}", "\\}")
            style_name, cue_metrics = style_for_time(start_ms / 1000)
            try:
                pos_x = float(cue_metrics.get("position_x_ratio"))
                pos_y = float(cue_metrics.get("position_y_ratio"))
            except (TypeError, ValueError):
                pos_x = pos_y = None
            if pos_x is not None and pos_y is not None:
                x = round(max(.03, min(.97, pos_x)) * width)
                y = round(max(.04, min(.96, pos_y)) * height)
                text = f"{{\\an{cue_metrics['alignment']}\\pos({x},{y})}}{text}"
            events.append(
                f"Dialogue: 0,{VideoCompose._ass_time(start_ms)},{VideoCompose._ass_time(end_ms)},"
        ...
        ass_lines = [
            ...
            style_line("Default", metrics),
            *(style_line(name, item) for _start, _end, name, item in scene_styles),
            "",
            "[Events]",
