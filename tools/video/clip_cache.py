...
cache of downloaded clip files at ``~/.haike_video/clips_cache/``.
...
- **LRU eviction at cap.** Default cap is 20 GB, overridable via
  ``HAIKE_VIDEO_CACHE_MAX_GB``. When ``ingest()`` would push total
...
# Default 20 GB cap. Overridable via HAIKE_VIDEO_CACHE_MAX_GB.
...
def default_cache_dir() -> Path:
    ...
    Honors ``HAIKE_VIDEO_CACHE_DIR`` if set, else falls back to
    ``~/.haike_video/clips_cache``. Does not create the directory —
    that happens in ``ClipCache.__init__`` on first use.
    """
    override = os.environ.get("HAIKE_VIDEO_CACHE_DIR")
    if override:
        return Path(override).expanduser()
    return Path.home() / ".haike_video" / "clips_cache"


def default_max_total_bytes() -> int:
    """Resolve the max-cache-size budget.

    Honors ``HAIKE_VIDEO_CACHE_MAX_GB`` (float or int) if set, else
    ...
    """
    override = os.environ.get("HAIKE_VIDEO_CACHE_MAX_GB")
    if override:
        try:
...
def reset_default_cache() -> None:
    ...
    re-reads env vars. Useful for tests that mutate ``HAIKE_VIDEO_CACHE_DIR``.
    """
    global _DEFAULT_CACHE
    _DEFAULT_CACHE = None
