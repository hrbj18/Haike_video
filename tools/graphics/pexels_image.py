...
class PexelsImage(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "exclude_photo_ids": {
                ...
                "description": "Pexels photo IDs that must not be selected",
            },
            "download_size": {
                "type": "string",
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            excluded_ids = {str(value) for value in inputs.get("exclude_photo_ids", []) if value is not None}
            photo = next((item for item in photos if str(item.get("id")) not in excluded_ids), None)
            if photo is None:
                return ToolResult(
                    success=False,
                    error=f"All returned images were already used for query: {query}",
                    data={
