...
class AudioMixer(BaseTool):
    ...
    input_schema = {
        ...
        "properties": {
            ...
            "source_start_seconds": {
                ...
                "description": "segmented_music only. First source second included in the loop.",
            },
            "source_end_seconds": {
                "type": "number", "exclusiveMinimum": 0,
                "description": "segmented_music only. Exclusive source end second; defaults to duration.",
            },
            "segments": {
                "type": "array",
                "description": (
            ...
            "fade_in_seconds": {
                "type": "number",
                "default": 0.5,
                "description": "segmented_music only. Fade-in duration in seconds.",
            },
            "fade_out_seconds": {
                "type": "number",
                "default": 0.5,
                "description": "segmented_music only. Fade-out duration in seconds.",
            },
            "target_duration": {
                "type": "number",
                "exclusiveMinimum": 0,
    ...
    def _segmented_music(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        try:
            ...
            return ToolResult(success=False, error="music_volume must be a number")
        if not math.isfinite(music_volume) or not 0.0 <= music_volume <= 1.0:
            return ToolResult(success=False, error="music_volume must be between 0 and 1")
        fade_dur = max(0.0, float(inputs.get("fade_duration", 0.5) or 0.0))
        fade_in_requested = max(0.0, float(inputs.get("fade_in_seconds", fade_dur) or 0.0))
        fade_out_requested = max(0.0, float(inputs.get("fade_out_seconds", fade_dur) or 0.0))
        ...
        music_dur = float(self.run_command(music_dur_cmd).stdout.strip().split("\n")[0])
        try:
            source_start = float(inputs.get("source_start_seconds", 0.0) or 0.0)
            raw_source_end = inputs.get("source_end_seconds")
            source_end = music_dur if raw_source_end is None else float(raw_source_end)
        except (TypeError, ValueError):
            return ToolResult(success=False, error="Music source range must contain numeric seconds")
        if not all(math.isfinite(value) for value in (music_dur, source_start, source_end)):
            return ToolResult(success=False, error="Music source range must contain finite seconds")
        if not (0.0 <= source_start < source_end <= music_dur + 0.001):
            return ToolResult(success=False, error="Music source range must satisfy 0 <= start < end <= duration")
        if source_end - source_start < 1.0 - 0.001:
            return ToolResult(success=False, error="Music source range must be at least 1 second")
        source_end = min(source_end, music_dur)

        # Build volume expression for each segment with smooth fades
        parts = []
        for seg in sorted(segments, key=lambda s: s["start"]):
            s = max(0.0, min(total_dur, float(seg["start"])))
            e = max(s, min(total_dur, float(seg["end"])))
            if e - s <= 0.001:
                continue
            # Never let the two ramps overlap into a malformed or surprising
            # expression on very short videos.
            ramp_budget = max(0.0, (e - s) / 2.0)
            fade_in = min(fade_in_requested, ramp_budget)
            fade_out = min(fade_out_requested, ramp_budget)
            fade_in_end = s + fade_in
            fade_out_start = e - fade_out
            fade_in_expr = (
                f"if(lt(t,{fade_in_end}),{music_volume}*(t-{s})/{fade_in},"
                if fade_in > 0.0001 else ""
            )
            fade_out_expr = (
                f"if(lt(t,{e}),{music_volume}*({e}-t)/{fade_out},0)"
                if fade_out > 0.0001 else f"if(lt(t,{e}),{music_volume},0)"
            )
            if fade_in_expr:
                parts.append(
                    f"if(lt(t,{s}),0,{fade_in_expr}"
                    f"if(lt(t,{fade_out_start}),{music_volume},{fade_out_expr})))"
                )
            else:
                parts.append(
                    f"if(lt(t,{s}),0,if(lt(t,{fade_out_start}),{music_volume},{fade_out_expr}))"
                )

        if not parts:
            return ToolResult(success=False, error="No valid music segments remain after clamping")

        vol_expr = "+".join(f"({p})" for p in parts) if len(parts) > 1 else parts[0]

        filter_complex = (
            f"[1:a]atrim=start={source_start}:end={source_end},asetpts=PTS-STARTPTS,"
            f"aloop=loop=-1:size=2147483647,atrim=0:{total_dur},"
            f"volume='{vol_expr}':eval=frame[music_shaped];"
            f"[0:a]aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo[speech];"
            f"[music_shaped]aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo[music_fmt];"
        ...
        return ToolResult(
            ...
            data={
                ...
                "fade_in_seconds": fade_in_requested,
                "fade_out_seconds": fade_out_requested,
                "output": str(output_path),
            },
            artifacts=[str(output_path)],
        )
