...
| **`news_anchor`** | **`NewsAnchor`** | **`text`** (headline), **`backgroundImage`/`backgroundVideo`** | **`kicker`, `avatarImage`, `avatarLabel`, `centerMotif`, `backgroundOverlay`, `accentColor`** | **深色科技新闻四固定位 — full-bleed footage + top-left chip headline + top-right avatar pip + bottom caption safe area. `text`=主标题, `kicker`=副标签(仅短标签，禁止填入分镜简报), `centerMotif`=`"question_marks"` 时画面中央叠一组动态问号(钩子镜头), `avatarImage`/`avatarLabel`=数字人口播占位。强调色唯一 `#22D3EE` 仅作短下划线。字幕文字不在此组件渲染——成片字幕由项目字幕系统按时序烧录，组件只保留底部无字对比垫层，避免双层字幕。** |
