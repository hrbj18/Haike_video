...
class TestDashscopeImageSpecific:
    ...
    def test_safe_error_redacts_key(self, monkeypatch):
        monkeypatch.setenv("DASHSCOPE_API_KEY", "TEST_DASHSCOPE_KEY_NOT_REAL")
        redacted = DashscopeImage._safe_error(
            Exception("failed with key TEST_DASHSCOPE_KEY_NOT_REAL")
        )
        assert "TEST_DASHSCOPE_KEY_NOT_REAL" not in redacted
...
class TestDashscopeRegistryDiscovery:
    ...
    def test_all_dashscope_tools_discoverable(self):
        ...
        # DashScope now also supplies the per-turn WAN S2V avatar provider.
        # Keep this assertion exact so a provider registration regression is
        # still visible instead of being hidden by a subset check.
        assert names == {"dashscope_image", "dashscope_tts", "dashscope_asr", "dashscope_avatar"}
