...
def test_architecture_env_mapping_includes_kling_official():
    ...
    # Product forks may rebrand the workbench while keeping the same provider
    # contract.  Assert the semantic statement instead of one product name.
    assert "Kling Official also adds provider tools only where" in architecture
    assert "already has a" in architecture
