...
def test_agent_guide_routes_to_runtime_hard_rule():
    """The fast router must point to the index, and the runtime meta-skill
    remains the authoritative hard rule."""
    guide = (ROOT / "AGENT_GUIDE.md").read_text(encoding="utf-8")
    assert "skills/INDEX.md" in guide

    selector = (SKILLS_DIR / "meta" / "animation-runtime-selector.md").read_text(encoding="utf-8")
    assert "HARD RULE" in selector
    assert "present both runtimes" in selector.lower()
    assert "silently" in selector.lower()
