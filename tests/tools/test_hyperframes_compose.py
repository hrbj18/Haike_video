...
def test_runtime_check_uses_local_cli_even_when_npm_is_offline(monkeypatch):
    """Once a local CLI exists, registry reachability is diagnostic only."""
    monkeypatch.setattr(HyperFramesCompose, "_node_major_version", classmethod(lambda cls: 24))
    monkeypatch.setattr("tools.video.hyperframes_compose.shutil.which", lambda name: f"mock-{name}")
    monkeypatch.setattr(
        HyperFramesCompose,
        "_resolve_npm_package",
        classmethod(lambda cls: {"error": "timeout (20s) — offline"}),
    )
    monkeypatch.setattr(
        HyperFramesCompose,
        "_probe_cli",
        classmethod(lambda cls: {"status": "ok", "version": "0.8.20", "source": "npm_npx_cache"}),
    )
    rc = HyperFramesCompose()._runtime_check()
    assert rc["runtime_available"] is True
    assert rc["npm_package_version"] == "0.8.20"
    assert rc["npm_resolve_error"] is None
    assert rc["cli_source"] == "npm_npx_cache"
    assert rc["reasons"] == []
...
def test_runtime_check_fails_when_local_cli_is_missing(monkeypatch):
    ...
    assert rc["reason_code"] == "hyperframes_cli_missing"
    assert "初始化" in rc["user_message"]


def test_runtime_check_fails_when_local_cli_crashes(monkeypatch):
    monkeypatch.setattr(HyperFramesCompose, "_node_major_version", classmethod(lambda cls: 24))
    monkeypatch.setattr("tools.video.hyperframes_compose.shutil.which", lambda name: f"mock-{name}")
    monkeypatch.setattr(
        HyperFramesCompose,
        ...
        classmethod(
            lambda cls: {
                "error": 'local CLI launch failed: The "file" argument must be of type string',
    ...
    assert rc["reason_code"] == "hyperframes_cli_launch_failed"


def test_doctor_uses_json_ok_instead_of_exit_code(monkeypatch):
    import subprocess

    tool = HyperFramesCompose()
    monkeypatch.setattr(tool, "_runtime_check", lambda: {"runtime_available": True, "reasons": []})
    monkeypatch.setattr(
        tool,
        "_run_hf",
        lambda *args, **kwargs: subprocess.CompletedProcess(
            args=["hyperframes", "doctor", "--json"],
            returncode=0,
            stdout='{"ok": false, "checks": [{"name": "ffmpeg", "ok": false}]}',
            stderr="",
        ),
    )
    result = tool._doctor({})
    assert result.success is False
    assert result.data["cli_doctor"]["report"]["ok"] is False
...
def test_video_compose_render_engines_follow_hyperframes_runtime_check(monkeypatch):
    ...
    monkeypatch.setattr(
        HyperFramesCompose,
        "_runtime_check",
        lambda self: {"runtime_available": False, "reasons": ["local CLI missing"]},
    )
...
def test_hyperframes_portrait_profile_never_silently_falls_back_to_landscape():
    assert HyperFramesCompose._resolve_dimensions("tiktok", 30)[:2] == (1080, 1920)
    # Legacy workbench artifacts are still interpreted as 9:16.
    assert HyperFramesCompose._resolve_dimensions("tiktok_vertical", 30)[:2] == (1080, 1920)
...
def test_hyperframes_render_reports_inspect_browser_failure_as_runtime_error(
    tmp_path, monkeypatch
):
    """A crashed browser is not a layout defect and must not prescribe CSS fixes."""
    from tools.base_tool import ToolResult
    ...
    monkeypatch.setattr(
        ...
        lambda inputs: ToolResult(
            success=False,
            error="hyperframes inspect exit 1",
            data={"report": {"error": "Browser process exited with code 3221225595"}},
        ),
    )
    monkeypatch.setattr(HyperFramesCompose, "_render_blocker_cache", None)

    result = tool._render(
        {
            "workspace_path": str(workspace),
            "edit_decisions": {"metadata": {"require_layout_inspect": True}},
        }
    )

    assert not result.success
    assert "browser process cannot start" in (result.error or "")
    assert "fix the layout" not in (result.error or "")
    assert HyperFramesCompose._render_blocker_cache
...
def test_scaffold_uses_frozen_tech_brief_style_composition(tmp_path: Path):
    workspace = tmp_path / "tech-brief"
    result = HyperFramesCompose().execute(
        {
            "operation": "scaffold_workspace",
            "workspace_path": str(workspace),
            ...
            "edit_decisions": {
                "version": "1.0",
                "cuts": [{"id": "scene", "type": "text_card", "text": "ignore", "in_seconds": 0, "out_seconds": 8}],
                "metadata": {
                    "title": "科技快报",
            ...
            "asset_manifest": {"assets": []},
            "playbook": {"name": "科技快报风格包 V1"},
        }
    )
    assert result.success, result.error
    html = (workspace / "index.html").read_text(encoding="utf-8")
    assert 'data-composition-id="tech-brief-v1"' in html
    assert 'class="clip"' in html
    assert 'tl.fromTo("#connector-line-0"' in html
    assert "数字人开始影响现实" in html
    assert "CAPTION SAFE AREA" not in html
    assert "这是当前字幕" not in html
    assert (workspace / "STYLE-PACK.json").is_file()
    assert 'src="assets/runtime/gsap.min.js"' in html
    assert "cdn.jsdelivr.net" not in html
    assert (workspace / "assets" / "runtime" / "gsap.min.js").is_file()
    assert "height:680px; z-index:2;" in html


@pytest.mark.parametrize("recipe,marker", [
    ("headline_statement", "headline-layout"),
    ("relationship_map", "relation-layout"),
    ("single_metric", "metric-layout"),
    ("comparison", "comparison-layout"),
    ("process", "process-layout"),
    ("quote_evidence", "evidence-layout"),
    ("closing_question", "question-layout"),
])
def test_tech_brief_recipes_render_distinct_information_structures(recipe, marker):
    composer = HyperFramesCompose()
    html = composer._generate_tech_brief_v1_index_html(
        style_context={
            "style_pack_id": "tech-brief-v1",
            "style_pack_version": "1.0.0",
            "aspect_profile": "portrait",
            "scene_recipe": recipe,
            "graphic_copy": {
                "scene_goal": "数字角色正在从形象变成可运营资产",
                "headline": "身份经营连接现实价值",
                "supporting_statement": "持续表达和用户关系让角色产生长期影响",
                "eyebrow": "T003 · 科技快报",
                "center_label": "数字角色",
                "nodes": ["固定形象", "粉丝关系", "宣传参与", "商业价值"],
            },
        ...
        width=1080, height=1920, total_duration=8,
        css_vars={"--font-heading": "sans-serif", "--font-body": "sans-serif", "--font-mono": "monospace"},
        title="科技快报",
    )
    assert marker in html
    assert f'data-recipe="{recipe}"' in html
    if recipe == "closing_question":
        assert 'id="question-mark" class="question-mark" data-layout-allow-occlusion' in html
    if recipe != "relationship_map":
        assert 'class="diagram relation-layout"' not in html


def test_tech_brief_landscape_process_reserves_space_for_four_steps_and_goal():
    html = HyperFramesCompose()._generate_tech_brief_v1_index_html(
        style_context={
            "style_pack_id": "tech-brief-v1",
            "style_pack_version": "1.0.0",
            "aspect_profile": "landscape",
            "scene_recipe": "process",
            "graphic_copy": {
                "scene_goal": "后续价值",
                "headline": "从想法到成品",
                "center_label": "流程",
    ...
    assert 'data-aspect="landscape"' in html
    assert html.count('class="process-step"') == 4
    assert '[data-aspect="landscape"] .process-layout { padding:10px 0 96px 30%; }' in html
    assert '[data-aspect="landscape"] .process-step { min-height:62px; margin:0 0 10px 66px;' in html
    assert '[data-aspect="landscape"] .process-goal { padding:15px 22px; font-size:22px;' in html


@pytest.mark.parametrize("recipe,variant,marker", [
    ("headline_statement", "signal_stack", "signal-stack-layout"),
    ("relationship_map", "causal_chain", "causal-chain-layout"),
    ("relationship_map", "convergence", "convergence-layout"),
    ("single_metric", "metric_ledger", "ledger-layout"),
    ("comparison", "stacked_duel", "stacked-duel-layout"),
    ("comparison", "balance_axis", "balance-axis-layout"),
    ("process", "zigzag_steps", "zigzag-layout"),
])
def test_tech_brief_layout_variants_emit_distinct_safe_compositions(recipe, variant, marker):
    """Every declared non-default layout must map to a real renderer branch.

    This guards against a planner offering a pretty selector that silently
    falls back to the old relationship-map geometry at render time.
    """
    composer = HyperFramesCompose()
    html = composer._generate_tech_brief_v1_index_html(
        style_context={
    ...
    assert marker in html
    assert f'data-recipe="{recipe}" data-layout="{variant}"' in html
    assert f"{recipe.upper()} / {variant.upper()}" in html
    assert 'class="caption-reserve"' in html


def test_tech_brief_renderer_defensively_falls_back_for_an_unknown_layout_variant():
    html = HyperFramesCompose()._generate_tech_brief_v1_index_html(
        style_context={
            "scene_recipe": "relationship_map",
            "layout_variant": "unknown-layout",
            "graphic_copy": {"headline": "测试", "center_label": "关系", "nodes": ["甲", "乙", "丙"]},
        },
        width=1080, height=1920, total_duration=5,
        css_vars={"--font-heading": "sans-serif", "--font-body": "sans-serif", "--font-mono": "monospace"},
        title="测试",
    )
    assert 'data-layout="radial_map"' in html
    assert 'class="diagram relation-layout"' in html


def test_daily_story_overlay_owns_headline_so_hyperframes_does_not_bake_duplicate():
    html = HyperFramesCompose()._generate_tech_brief_v1_index_html(
        style_context={
            "scene_recipe": "relationship_map",
    ...
    assert "不应被HY重复渲染的新闻标题" not in html
    assert 'id="headline"' not in html
    assert 'tl.from("#headline"' not in html
