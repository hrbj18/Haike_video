...
def test_video_compose_surfaces_all_three_runtimes():
    ...
    assert engines["ffmpeg"] is bool(video_compose_module._discover_ffmpeg_pair())
    assert "hyperframes_note" in info
    assert "runtime_governance" in info


def test_video_compose_ffmpeg_engine_requires_complete_pair(monkeypatch):
    """A lone ffmpeg or a guessed install must never pass proposal preflight."""
    import tools.video.video_compose as video_compose_module

    monkeypatch.setattr(video_compose_module, "_discover_ffmpeg_pair", lambda: None)
    info = VideoCompose().get_info()
    ...
    monkeypatch.setattr(
        ...
        lambda: ("/runtime/ffmpeg", "/runtime/ffprobe"),
    )
    info = VideoCompose().get_info()
    assert info["render_engines"]["ffmpeg"] is True


def test_media_runtime_contract_pins_static_pair_and_preflight_checks_probe():
    import ast

    repo_root = Path(__file__).resolve().parents[2]
    requirements = (repo_root / "requirements.txt").read_text(encoding="utf-8")
    preflight = (repo_root / "scripts" / "preflight.ps1").read_text(encoding="utf-8")
    compose_source = (repo_root / "tools" / "video" / "video_compose.py").read_text(encoding="utf-8")

    assert "static-ffmpeg==3.0" in requirements
    assert "static_ffmpeg\\bin\\win32" in preflight
    assert "ffprobe" in preflight
    assert "static_ffmpeg.add_paths(" not in preflight
    calls = [
        node for node in ast.walk(ast.parse(compose_source))
        if isinstance(node, ast.Call)
        and isinstance(node.func, ast.Attribute)
        and node.func.attr == "add_paths"
    ]
    assert calls == []
