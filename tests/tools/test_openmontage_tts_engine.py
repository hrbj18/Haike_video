...
def test_cpu_model_load_uses_memory_conservative_bfloat16_unless_explicitly_overridden(tmp_path, monkeypatch):
    import tools.audio.openmontage_tts_engine as engine_module
    ...
    QwenTTSRuntime(tmp_path)._load_model("base")
    assert calls[-1]["dtype"] == "bfloat16"

    monkeypatch.setenv("OPENMONTAGE_TTS_CPU_DTYPE", "float32")
    QwenTTSRuntime(tmp_path)._load_model("base")
