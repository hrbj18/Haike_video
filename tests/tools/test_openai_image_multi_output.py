...
def test_uses_configured_openai_compatible_base_url(openai_tool, monkeypatch, tmp_path):
    monkeypatch.setenv("OPENAI_BASE_URL", "https://gateway.example/v1")

    result = openai_tool.execute({"prompt": "p", "output_path": str(tmp_path / "configured.png")})

    assert result.success
    assert _FakeClient.last_kwargs == {"base_url": "https://gateway.example/v1"}
    assert result.data["base_url_configured"] is True
