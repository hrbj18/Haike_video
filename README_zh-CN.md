...
- **Windows 10/11 x64** 与 PowerShell 5.1+
- **64 位 Python 3.12** — [python.org](https://www.python.org/downloads/)
- **Node.js 22+**（包含 npm）— [nodejs.org](https://nodejs.org/)
- **Git for Windows** — [git-scm.com](https://git-scm.com/download/win)
- **一款 AI 编程助手** — Claude Code、Cursor、Copilot、Windsurf 或 Codex

FFmpeg/ffprobe 由项目锁定的 Python 依赖提供。完整的跨电脑配置、私有素材迁移和验收边界见 [Windows 部署指南](docs/DEPLOYMENT_WINDOWS_ZH-CN.md)。
...
> `scripts\setup.ps1` 会创建 Python 3.12 虚拟环境、安装核心/测试/faster-whisper 依赖、执行锁文件驱动的 `npm ci`，并配置独立的 OpenMontage 本地 TTS 运行时。真实密钥只写入自动创建的 `.env.local`，不要提交到 Git。

### Windows 一键启动工作台

完成一次依赖安装后，直接双击项目根目录的 [`启动工作台.bat`](启动工作台.bat)。它会自动启动本地工作台（若已运行则直接复用），并打开浏览器中的项目库页面。

### Windows 内置本地配音

本项目可用仓库自带的 Qwen3-TTS 服务完成中文预设和克隆音色配音，不需要另外部署 Voicebox。首次安装运行：

```powershell
powershell.exe -NoProfile -ExecutionPolicy Bypass -File scripts\setup_local_tts.ps1
```

依赖、模型和私有克隆音色都位于被 Git 忽略的 `.backlot`，工作台启动时会自动拉起本地服务。旧机器已有雅雅/檬檬音色时，可加 `-MigrateVoicebox` 一次性迁入；换电脑用私有音色包导入，不需要重新安装 Voicebox。完整说明见 [`docs/LOCAL_TTS_ZH-CN.md`](docs/LOCAL_TTS_ZH-CN.md)。
