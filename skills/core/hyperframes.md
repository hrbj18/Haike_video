...
This is the **Haike Video-specific** guide to HyperFrames. It explains when
Haike Video pipelines should choose HyperFrames over Remotion, how Haike Video
...
## When Haike Video should pick HyperFrames (vs Remotion vs FFmpeg)

Haike Video separates two concepts:
...
| Synthetic UI / fake terminal / fake browser demo | Either — depends on existing coverage | Haike Video already ships Remotion `TerminalScene` (see `synthetic-screen-recording` Layer 3). For UI chrome beyond terminal, HyperFrames HTML is easier. |
...
Haike Video artifacts into HyperFrames project files:

| Haike Video artifact field | HyperFrames target |
|---|---|
...
`STORYBOARD.md` as step-by-step workspace files. Haike Video does **not**
...
Haike Video playbooks currently translate into Remotion `themeConfig`
...
- ❌ Running `hyperframes init` from the Haike Video orchestrator. `init`
