# ComfyUI Provider Adapter for Haike Video
...
Haike Video's local GPU tools (`wan_video`, `hunyuan_video`, `cogvideo_video`,
...
A ComfyUI adapter gives Haike Video access to any model ComfyUI supports,
...
**Future path:** ACE-Step support should be revisited once Haike Video decides
...
URL when Haike Video knows the canonical source. Agents should surface that
...
using Haike Video's 7-dimension scoring:
...
Haike Video should treat those as custom workflow profiles until a blessed
...
### Future (add models to ComfyUI, no code changes to Haike Video)
...
4. **Music generation:** ACE-Step works in ComfyUI but Haike Video needs a
