...
豆包可切换配音已形成独立候选：配音中心可管理本地 Qwen3-TTS 与豆包音色；无数字人、片段候选和数字人驱动音频统一冻结供应商与音色；云端结果转为 24kHz 单声道 PCM WAV 并保留时间戳。API Key 仅存于 Git 忽略文件；公版 Speech 2.0 的小棓女声和开朗学长男声真实短样本均成功。雅雅 / 檬檬的 `S_` 复刻音色因当前账号未授权 ICL 资源仍为 `partial`。
## 固定生产合同
...
- 云端 TTS 独立候选的工具、配音中心、任务冻结和数字人路由回归共 166 passed。此前全量基线为 1781 passed、52 skipped；已知环境失败位于 FFmpeg、浏览器权限或 Google 可选后端测试组。
- 真实豆包试听：`seed-tts-2.0` 下小棓女声 6.67 秒 / 133,692 字节、开朗学长男声 4.82 秒 / 96,732 字节；两者均为终态 `2`、1 句 / 21 个字级时间点，解码通过。私有 `S_` 音色在创建任务前因资源未授权失败，没有重复付费任务。
- 发布候选 1726 个文件、约 27.7 MiB；没有大于 1 MiB 的文件、真实环境文件、高置信密钥、本机路径、私有媒体或付费产物。
- 完成态幂等恢复没有新增 worker、operation 或费用。

## 下一步

1. 将云端 TTS 独立发布候选部署到广州测试服务器，完成无付费的健康、供应商和本地 Qwen 回退检查。
2. 如果要使用雅雅 / 檬檬的 `S_` 复刻音色，先在同一火山引擎项目开通与音色版本匹配的声音复刻资源，再对每个音色各执行一条短样本。未开通前使用已验证的公版云音色或本地 Qwen3-TTS。
3. 其他新开发仍从 `dev` 创建 `codex/<任务名>`，通过集成验收后再发布到 `main`；不得把共享工作区整树推送。
4. 私下迁移音色、人物图和本地环境配置；不要经 GitHub 同步。
