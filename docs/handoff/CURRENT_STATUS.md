...
可复现源码、测试、公开配置模板和交接文档已经首次发布到私有仓库 `https://github.com/hrbj18/haike` 的 `main`。初始审计快照提交为 `3c35019195ac023c9a88008b9128f0a31ba4a7dd`；本机开发 `main` 仍跟随 upstream，发布引用单独保存在 `release-main`，未覆盖当前脏工作树。密钥、私有音色、人物图、音乐、模型、缓存、项目媒体和付费产物均未进入 Git。
...
- 精简发布快照正式套件：1780 passed, 24 skipped, 1 subtest passed；修复公开脚本模板夹具后受影响回归 8 passed，个人素材测试按预期跳过 5 项。
...
1. 在另一台 Windows 电脑按 `docs/DEPLOYMENT_WINDOWS_ZH-CN.md` 完成私有仓库克隆、干净安装与无付费回归。
