...
豆包可切换配音已形成独立候选：配音中心可管理本地 Qwen3-TTS 与豆包音色；无数字人、片段候选和数字人驱动音频统一冻结供应商与音色；云端结果转为 24kHz 单声道 PCM WAV 并保留时间戳。API Key 仅存于 Git 忽略文件；公版 Speech 2.0 真实短样本已成功。雅雅 / 檬檬的 `S_` 复刻音色因当前账号未授权 ICL 资源仍为 `partial`。
## 固定生产合同
...
- 真实豆包试听：公版女声 + `seed-tts-2.0` 成功，产物 69,852 字节，任务终态 `2`，返回 1 句 / 14 个字级时间点；请求与合成文本长度均为 16。私有 `S_` 音色均在创建任务前因资源未授权失败，没有重复付费任务。
