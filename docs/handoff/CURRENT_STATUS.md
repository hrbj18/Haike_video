...
准备把可复现的源码、测试、公开配置模板和交接文档发布到私有仓库 `https://github.com/hrbj18/haike`。密钥、私有音色、人物图、音乐、模型、缓存、项目媒体和付费产物不进入 Git。
...
1. 显式提交已经审计的精简发布快照并推送到 `origin/main`，不得使用 `git add .` 或裸 `git push`。
2. 推送后核验远端默认分支和部署文档，再把发布提交写回本状态。
3. 在另一台 Windows 电脑按部署文档完成一次干净安装与无付费回归；真实供应商验证另行人工授权。
