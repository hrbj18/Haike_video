...
- 密钥、私有音色、人物参考图、音乐、项目媒体、模型缓存和付费产物不在 GitHub 中；同事可按核心密钥交接文档，用隐藏输入脚本一次配置 GPT 中转站、豆包文本与 RunningHub 凭据。

本机历史工作区中存在的后续素材理解、自动选材等未发布开发，不属于 `v0.1.1`，不能仅凭本机演示宣称 GitHub 稳定版已经具备。

## 接手与发布

- 新同事从仓库根目录 [START_HERE.md](../../START_HERE.md) 开始。
- 安装按 [Windows 部署指南](../DEPLOYMENT_WINDOWS_ZH-CN.md)。
- 密钥按 [核心密钥配置指令](../CORE_SECRETS_CODEX_HANDOFF_ZH-CN.md)，真实值不进入文档或 Git。
